...
@media (prefers-color-scheme: light) {
    #cabecalho-fixo { background: rgba(255,255,255,0.88); border-bottom: 1px solid rgba(0,0,0,0.08); }
}
</style>
...
# ================================================================
# EXIBIÇÃO — TABELA COMPARATIVA
# Colunas: Label | 2025 | 2026 Jan-Mar (Real) | 2026 Abr-Dez (Proj)
# Para adicionar 2024: descomente as linhas marcadas com "# ← 2024"
# e altere COLS para [2.2, 1, 1, 1, 1]
# ================================================================
...
# Para ocultar a coluna 2026 Total Ano: mude SHOW_2026_TOTAL para False
SHOW_2026_TOTAL = True

# ── Visibilidade de colunas ──────────────────────────────────────
# Mude qualquer flag para False para ocultar a coluna correspondente.
SHOW_COL_2024  = True
SHOW_COL_2025  = True
SHOW_COL_26R   = True   # 2026 Jan-Mar
SHOW_COL_26P   = True   # 2026 Abr-Dez
# SHOW_2026_TOTAL já controla a última coluna
...
def render_secao(titulo, bg_sec=BG_NEUTRO, cor_borda="#888", expandido=True, key=None):
    """Renderiza cabeçalho de seção. Retorna True se expandida."""
    ...
    html = (
        ...
        f"letter-spacing:0.1em;'>{icone} {titulo}</span></div>"
    )
    st.markdown(html, unsafe_allow_html=True)
    if key:
        # botão invisível para toggle (sobreposto ao header via CSS trick com st.button)
        pass
...
with cols[0]:
    st.markdown(
        f"<div style='padding:8px 12px; margin:2px 0;'>"
...
with cols[1]:  # 2024
    st.markdown(
        f"<div style='border-left:4px solid {ACENTO_24}; background:{BG_NEUTRO}; padding:8px 12px; border-radius:6px; text-align:center; margin:2px 0;'>"
        f"<span style='color:{ACENTO_24}; font-weight:bold; font-size:1.05rem;'>2024</span><br>"
        f"<span style='font-size:0.7rem; opacity:0.55;'>Fechado</span></div>",
        unsafe_allow_html=True)
with cols[2]:
    st.markdown(
        f"<div style='border-left:4px solid {ACENTO_25}; background:{BG_NEUTRO}; padding:8px 12px; border-radius:6px; text-align:center; margin:2px 0;'>"
        f"<span style='color:{ACENTO_25}; font-weight:bold; font-size:1.05rem;'>2025</span><br>"
        f"<span style='font-size:0.7rem; opacity:0.55;'>Fechado</span></div>",
        unsafe_allow_html=True)
with cols[3]:
    st.markdown(
        f"<div style='border-left:4px solid {ACENTO_26R}; background:{BG_NEUTRO}; padding:8px 12px; border-radius:6px; text-align:center; margin:2px 0;'>"
        f"<span style='color:{ACENTO_26R}; font-weight:bold; font-size:1.05rem;'>2026 Jan-Mar</span><br>"
        f"<span style='font-size:0.7rem; opacity:0.55;'>Realizado</span></div>",
        unsafe_allow_html=True)
with cols[4]:
    st.markdown(
        f"<div style='border-left:4px solid {ACENTO_26P}; background:{BG_NEUTRO}; padding:8px 12px; border-radius:6px; text-align:center; margin:2px 0;'>"
        f"<span style='color:{ACENTO_26P}; font-weight:bold; font-size:1.05rem;'>2026 Abr-Dez</span><br>"
        f"<span style='font-size:0.7rem; opacity:0.55;'>Projetado</span></div>",
        unsafe_allow_html=True)
if SHOW_2026_TOTAL:
    with cols[5]:
        st.markdown(
            f"<div style='border-left:4px solid {ACENTO_26T}; background:rgba(46,125,50,0.18); padding:8px 12px; border-radius:6px; text-align:center; margin:2px 0;'>"
            f"<span style='color:{ACENTO_26T}; font-weight:bold; font-size:1.05rem;'>2026 Total</span><br>"
            f"<span style='font-size:0.7rem; opacity:0.55;'>Jan-Dez (est.)</span></div>",
            unsafe_allow_html=True)

# ---------- RECEITA ----------
render_secao("Receita", bg_sec=BG_SEC_FAT, cor_borda="#2e7d32")
render_row("Faturamento",
           FAT_2024, FAT_2025, FATURAMENTO_JAN_MAR, fat_abr_dez, FAT_2026,
           fmt="R$", bg_label=BG_FAT, bold=True)

# ---------- CUSTOS ----------
render_secao("Custos e Despesas", bg_sec=BG_SEC_CUS, cor_borda="#b45a00")
render_row("Custo Campo",      CUSTO_CAMPO_2024, CUSTO_CAMPO_2025, CUSTO_CAMPO_JAN_MAR, custo_campo_abr_dez, CUSTO_CAMPO_2026, fmt="R$", bg_label=BG_CUSTO)
render_row("Custo PH",         CUSTO_PH_2024,    CUSTO_PH_2025,    CUSTO_PH_JAN_MAR,    custo_ph_abr_dez,    CUSTO_PH_2026,    fmt="R$", bg_label=BG_CUSTO)
render_row("Adm. Sede",        CUSTO_ADM_2024,   CUSTO_ADM_2025,   CUSTO_ADM_JAN_MAR,   custo_adm_abr_dez,   CUSTO_ADM_2026,   fmt="R$", bg_label=BG_CUSTO)
render_row("Desp. Financeira", DESP_FIN_2024,    DESP_FIN_2025,    DESP_FIN_JAN_MAR,    desp_fin_abr_dez,    DESP_FIN_2026,    fmt="R$", bg_label=BG_CUSTO)
render_row("Total Custos",     CUSTOS_2024,      CUSTOS_2025,      CUSTO_TOTAL_JAN_MAR, custos_abr_dez,      CUSTOS_2026,      fmt="R$", bg_label=BG_TOTALC, bold=True)

# ---------- RESULTADO ----------
render_secao("Resultado", bg_sec=BG_SEC_RES, cor_borda="#1565c0")
...
# ---------- OPERACIONAL ----------
render_secao("Operacional", bg_sec=BG_SEC_OPE, cor_borda="#455a64")
...
st.divider()
# st.caption(
    # "Os dados de 2024 e 2025 são valores placeholder. "
    # "Substitua as constantes marcadas com # ← no início do arquivo pages/comparativo.py."
# )
