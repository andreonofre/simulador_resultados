...
(function () {
  ...
  // Aplica position:sticky ao primeiro stHorizontalBlock (o cabeçalho de colunas)
  // Também corrige overflow nos ancestrais
  function aplicarSticky() {
    var blocks = parent.document.querySelectorAll('[data-testid="stHorizontalBlock"]');
    if (!blocks.length) return;
    var header = blocks[0];
    // Corrige overflow em todos os ancestrais
    var el = header.parentElement;
    while (el && el.tagName !== 'BODY') {
      var ov  = parent.window.getComputedStyle(el).overflow;
      var ovY = parent.window.getComputedStyle(el).overflowY;
      if (ov === 'hidden' || ovY === 'hidden' || ov === 'auto' || ovY === 'auto') {
        el.style.setProperty('overflow', 'visible', 'important');
      }
      el = el.parentElement;
    }
    // Aplica sticky ao header
    header.style.setProperty('position', 'sticky', 'important');
    header.style.setProperty('top', '0', 'important');
    header.style.setProperty('z-index', '999', 'important');
    header.style.setProperty('background', 'rgba(14,17,23,0.97)', 'important');
  ...
  // MutationObserver seguro: só observa childList (adição/remoção de nós)
  // aplicarBotoes/aplicarSticky apenas modificam estilos (atributos), não childList
  // portanto não causam loop
